...
Extract discrete business operations into invokable Action classes that extend `App\Actions\Action`. This base class provides standardized container resolution and DTO handling.

### Implementing an Action

- Extend `App\Actions\Action`.
- Use the `@template` tag for the DTO if applicable.
- Define logic in the `__invoke()` method.
- Use `protected bool $requiresData = true` to enforce DTO presence.
...
/**
 * @extends Action<UserRegistrationDTO>
 */
class RegisterUser extends Action
{
    protected bool $requiresData = true;

    public function __invoke(): User
    {
        // $this->data is typed to UserRegistrationDTO
        return User::create($this->data->toArray());
    }
}
```
...
Always use `static::make()` to resolve the action from the container, allowing for dependency injection. Use `withData()` to pass the DTO, and `execute()` to run the logic.

```php
$user = RegisterUser::make()
    ->withData(new UserRegistrationDTO(...))
    ->execute();
```
